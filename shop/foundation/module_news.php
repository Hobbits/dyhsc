...
function get_news_cat_lists(&$dbo,$table) {
	$sql = "select * from `$table`";
	$result = $dbo->getRs($sql);
	$array = array();
	foreach($result as $value) {
		$array[$value['cat_id']]['cat_id'] = $value['cat_id'];
		$array[$value['cat_id']]['cat_name'] = $value['cat_name'];
		$array[$value['cat_id']]['art_num'] = $value['art_num'];
	}
	return $array;
}
...
function get_news_infos(&$dbo,$table,$article_id) {
	$sql = "select article_id,title,content,admin_id,add_time from `$table` where article_id='$article_id'";
	return $dbo->getRow($sql);
}
